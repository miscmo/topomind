...
核心功能：知识卡片房间模型，支持无限嵌套层级、双击钻入/退出、拖拽编辑、多类型文档编辑。

**关键架构约束（must-follow）**：
- Electron 主进程（IPC、文件系统、日志）完全复用，不改动
...
- 禁止直接在渲染进程调用 Electron IPC（通过 `useStorage()` 等 hooks 封装）
...
| Store | 文件 | 职责 |
|-------|------|------|
| `appStore` | `src/stores/appStore.ts` | 应用视图、选中节点、边模式、网格显示、右侧面板状态 |
...
**文件系统**：`fs:listChildren` / `fs:mkDir` / `fs:rmDir` / `fs:renameKB` / `fs:readGraphMeta` / `fs:writeGraphMeta` / `fs:getDir` / `fs:updateCardMeta` / `fs:readFile` / `fs:writeFile` / `fs:deleteFile` / `fs:countChildren` / `fs:isValidWorkDir` / `fs:selectDirectory` / `fs:createWorkDir` / `fs:importKB`
...
4. **房间切换**：通过 `tabStore` 的 `enterRoomInTab` / `goBackInTab` / `navigateToHistoryIndexInTab` 管理每个 KB Tab 的房间状态
5. **防抖保存**：修改节点或边后 debounce 300ms 自动保存，房间切换前需要 flush
6. **避免 stale closure**：异步回调、事件回调、Promise 链中优先通过 store 的 `getState()` 或 ref 读取最新值
7. **normalizeMeta**：已在 `saveLayout` 内部处理，调用方不要重复调用
8. **日志规范并不完全统一**：渲染层应优先走 `logger` / `logAction`，但 preload 中仍存在少量 `console.warn/error`，如需统一要同时改动 Electron 层
