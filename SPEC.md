...
└── src/
    ...
    │   ├── logger.js            # 日志工具（logger.catch 记录带堆栈的异常）
    │   └── log-backend.js       # 日志 IPC 桥接（读写主进程日志服务）
    ├── hooks/
...
| `src/core/log-backend.js` | 日志 IPC 桥接。封装日志读写、实时订阅、`logAction` 快捷构造器 |
...
| 文件 | 职责 |
|------|------|
| `src/core/log-backend.js` | 渲染进程 IPC 桥接，封装日志读写与实时订阅 |
...
#### 日志 API（src/core/log-backend.js）
