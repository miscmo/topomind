...
```js
// Windows 文件名不能以点或空格结尾，这里做额外清理
var s = raw.replace(/[. ]+$/g, '')
```

### 10.3 文档生成建议

如需将注释直接生成 API 文档，推荐采用：

- JavaScript 项目：`JSDoc`
- TypeScript 项目：`TypeDoc`

当前项目以 JavaScript / Vue / Electron 为主，因此默认采用 `JSDoc` 作为可生成文档的注释标准。

### 10.4 编写原则

- 优先注释“原因”“约束”“副作用”，而不是逐行翻译代码
- 不要为显而易见的变量赋值、循环、条件分支添加废话型注释
- 模块级能力可在文件顶部补充模块说明
- 对内部私有但高风险或高复用函数，也应补充简明 `JSDoc`
- 风格保持简洁统一，避免中英混杂和口语化过强的描述

### 10.5 落地建议

项目内推荐采用以下规则：

- `src/core/`、`electron/` 中的核心函数优先补齐 `JSDoc`
- `src/composables/` 对外暴露的方法统一补齐 `JSDoc`
- Vue 组件内部的局部交互逻辑按需使用普通注释
- 新增公共函数时，至少补充用途、参数、返回值三项说明
