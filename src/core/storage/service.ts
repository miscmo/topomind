...
export function createStore(adapter: StorageAdapter = fileStorageAdapter) {
  ...
  const store = {
    init() { try { return adapter.initWorkspace() } catch (e) { logger.catch('Store.init', '初始化工作目录失败', e); throw e } },
    setWorkDir(dirPath: string): Promise<DirDialogResult> { try { return adapter.setWorkspace(dirPath) as Promise<DirDialogResult> } catch (e) { logger.catch('Store.setWorkDir', '设置工作目录失败', e); throw e } },
    selectWorkDirCandidate(): Promise<DirDialogResult> { try { return adapter.selectWorkspaceCandidate() as Promise<DirDialogResult> } catch (e) { logger.catch('Store.selectWorkDirCandidate', '选择工作目录候选失败', e); throw e } },
    createWorkDir(dirPath: string): Promise<DirDialogResult> { try { return adapter.createWorkspace(dirPath) as Promise<DirDialogResult> } catch (e) { logger.catch('Store.createWorkDir', '创建工作目录失败', e); throw e } },
    ...
    listCards(parentPath: string) { try { return adapter.listCards(parentPath) } catch (e) { logger.catch('Store.listCards', `列出卡片失败: ${parentPath}`, e); throw e } },
    ...
    async clearAll() { try { return await adapter.clearWorkspace() } catch (e) { logger.catch('Store.clearAll', '清除所有数据失败', e); throw e } },
    ...
    async getRootDir(): Promise<string | null> { try { return await adapter.getWorkspaceRoot() } catch (e) { logger.catch('Store.getRootDir', '获取根目录失败', e); throw e } },
    ...
    async readConfig(): Promise<WorkDirConfig> { const now = Date.now(); if (cachedConfigTimestamp && now - cachedConfigTimestamp < CONFIG_CACHE_TTL) return cachedConfig; try { cachedConfig = normalizeConfig(await adapter.readAppConfig()); cachedConfigTimestamp = now; return cachedConfig } catch { cachedConfig = normalizeConfig({}); cachedConfigTimestamp = now; return cachedConfig } },
    async writeConfig(config: WorkDirConfig) { try { const next = normalizeConfig({ ...cachedConfig, ...config, defaultEdgeStyle: { ...cachedConfig.defaultEdgeStyle, ...config.defaultEdgeStyle } }); cachedConfig = next; return await adapter.writeAppConfig(next) } catch (e) { logger.catch('Store.writeConfig', '保存工作目录配置失败', e); throw e } },
  }
  return store
}
