...
import { LearningQuickPreview } from './LearningQuickPreview.tsx'
...
export const LearningTrackerWidget = memo(function LearningTrackerWidget({
  ...
  const { isActive, todayDuration } = useLearningRuntimeState(learning)
    ...
    >
      ...
      >
        <span className="relative flex h-2 w-2">
          <span className={`relative inline-flex rounded-full h-2 w-2 ${isActive ? 'bg-[var(--color-primary)]' : 'bg-[var(--color-text-muted)]'}`} />
        </span>
      ...
      {previewOpen && (
        <div className="absolute top-[calc(100%+8px)] left-0 min-w-[320px] bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl shadow-[var(--shadow-popover)] z-[4000] p-4 animate-in fade-in slide-in-from-top-2">
          <LearningQuickPreview learning={learning} workspace={workspace} />
        </div>
      )}
    </div>
  )
