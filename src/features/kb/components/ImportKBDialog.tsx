...
export function ImportKBDialog(props: ImportKBDialogProps) {
  const { visible, dir, loading, selectingDir = false, error, onClose, onSelectDir, onSubmit } = props
  ...
  return (
    <div
      onKeyDown={(e) => {
        if (e.key === 'Escape' && !loading && !selectingDir) onClose()
      }}
      className={`${modalOverlayBaseClassName} ${modalOverlayEnterClassName} z-[10000]`}
      onClick={(e) => { if (!loading && !selectingDir && e.target === e.currentTarget) onClose() }}
    >
      <div className={`w-[440px] max-w-[90%] overflow-hidden rounded-2xl border border-[var(--color-border)] bg-surface shadow-[var(--shadow-lg)] ${modalPanelEnterClassName}`} role="dialog" aria-modal="true" aria-labelledby="import-kb-title">
        <div className="p-[18px_24px] bg-[var(--color-bg)] border-b border-[var(--color-border-light)] flex justify-between items-center [&>h3]:text-[var(--color-primary)] [&>h3]:text-[16px] [&>h3]:m-0 [&>h3]:font-bold">
          <h3 id="import-kb-title">导入知识库</h3>
          <button type="button" aria-label="关闭导入知识库弹窗" className="w-7 h-7 rounded-md border-none bg-[var(--color-hover-bg)] text-[var(--color-text-muted)] cursor-pointer text-[14px] transition-all duration-75 hover:bg-[var(--color-bg-muted)] hover:text-[var(--color-text-primary)] disabled:opacity-50 disabled:cursor-not-allowed" onClick={onClose} disabled={loading || selectingDir}>✕</button>
        </div>
