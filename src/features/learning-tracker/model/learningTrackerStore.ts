...
export const useLearningTrackerStore = create<LearningTrackerState>((set, get) => {
  ...
  const resolveRootDir = (rootDirOverride?: string | null) => rootDirOverride ?? useWorkspaceStore.getState().currentWorkDir
  ...
  const doFlush = async (rootDirOverride?: string | null) => {
    ...
    try {
      ...
      for (const dateStr of affectedDates) {
        let dayData = await FSB.readLearningStatsData(rootDir, dateStr) as DailyRecord
      ...
      for (const [dateStr, dayData] of dayRecords.entries()) {
        await FSB.writeLearningStatsData(rootDir, dateStr, dayData)
      }

      const allDailyRecords = await FSB.readAllLearningStatsData(rootDir) as Record<string, DailyRecord>
  ...
  return {
    ...
    init: async () => {
      ...
      try {
        const rawMeta = normalizeMeta(await FSB.readLearningStatsData(rootDir) as LearningStatsMeta)
        const allDailyRecords = await FSB.readAllLearningStatsData(rootDir) as Record<string, DailyRecord>
        const meta = {
          ...rawMeta,
        ...
        let todayData = await FSB.readLearningStatsData(rootDir, todayStr) as DailyRecord
        if (!todayData || !todayData.date) {
          todayData = { date: todayStr, totalDuration: 0, sessions: [] }
        }

        await FSB.writeLearningStatsData(rootDir, null, meta)
