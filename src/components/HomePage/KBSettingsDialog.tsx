...
export function KBSettingsDialog({ visible, kb, onClose, refreshKBList }: KBSettingsDialogProps) {
  ...
  return (
    <div inert={!visible ? "" : undefined} className={`fixed inset-0 bg-black/40 z-[10000] flex items-center justify-center opacity-0 invisible transition-all duration-250 backdrop-blur-[2px] ${visible ? '!opacity-100 !visible [&>div]:!scale-100 [&>div]:!translate-y-0 [&>div]:!opacity-100' : ''}`} onClick={onClose}>
      <div className="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl w-[440px] max-w-[90%] overflow-hidden scale-95 translate-y-2.5 opacity-0 transition-all duration-75 shadow-[var(--shadow-lg)]" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-[20px_24px]">
          ...
          <div className="mb-4 [&>label]:block [&>label]:text-[var(--color-text-secondary)] [&>label]:text-[13px] [&>label]:mb-1.5 [&>label]:font-medium [&>input]:w-full [&>input]:p-[10px_14px] [&>input]:border [&>input]:border-[var(--color-border)] [&>input]:rounded-lg [&>input]:bg-[var(--color-surface)] [&>input]:text-[var(--color-text-primary)] [&>input]:text-[13px] [&>input]:transition-all [&>input]:duration-75 [&>input]:box-border focus:[&>input]:outline-none focus:[&>input]:border-[var(--color-accent)] focus:[&>input]:shadow-[0_0_0_2px_var(--color-accent-soft)] [&>input::placeholder]:text-[var(--color-text-muted)]">
            <label>封面设置</label>
            <label className="w-full h-[140px] bg-[var(--color-bg-muted)] rounded-xl overflow-hidden relative cursor-pointer flex items-center justify-center border border-[var(--color-border)] transition-all duration-75 hover:border-[var(--color-accent)] group">
              <input
                type="file"
                accept="image/*"
                className="hidden"
              ...
              <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 transition-opacity duration-75 text-white text-[14px] font-medium backdrop-blur-[2px] group-hover:opacity-100">
                点击替换封面图
              </div>
            </label>
          </div>
          ...
          <div className="mt-6 pt-5 border-t border-[var(--color-border-light)] flex justify-between items-center">
            ...
            <div className="flex gap-3">
              ...
              <button className="bg-[var(--color-accent)] text-white border-none p-[8px_24px] rounded-lg text-[14px] font-semibold cursor-pointer transition-all duration-75 hover:bg-[var(--color-accent-hover)] disabled:opacity-60 disabled:cursor-not-allowed" onClick={handleSave} disabled={loading || !name.trim()}>
                保存设置
              </button>
            </div>
          </div>
