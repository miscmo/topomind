...
```javascript
{
  "children": {
    ...
    },   // key = 全局唯一id：md5{卡片路径+时间戳}，name = 显示名称（可不同）
    "card-322312112": { 
      "name": "另一个名称" 
      "posX": 725.8836742991465,
      "posY": 46.00401123106391
...
- key为工作目录下的全局唯一，生成的思路是卡片在工作目录下的相对路径的md5，再追加一个时间戳，防止重复或者删除后重建的重复，使用id还有个好处，方便迁移到不同的存储，后面假如存储发生变化，比如用数据库存储知识库，那就
...
4. **导航状态语义复杂**
   - `tabStore` 中每 Tab 房间历史、当前房间和脏状态的边界仍需要谨慎维护。
   - 建议继续简化 tab/room 导航状态机。
...
| v5.3.1 | 2026-04-23 | **Bug 修复与架构净化**：修复 `navigateBack` 非 Tab 模式下 `loadRoom` 未被调用的回归；修复 `applyNodeRemoveChanges` 未调用 `scheduleSave` 导致节点移除后布局丢失；修复 `DetailPanel.handleDelete` 中 stale closure（异步回调引用过期 selectedNode）；修复 `edgesMapRef` 在 `updateEdgeRelation` 中未更新的不一致问题；修复 `applyNodeRemoveChanges` 未清理已删除节点连线导致的 orphaned edges；移除 `graphOperations.ts` 中的无用 import（`Node`、`KnowledgeNodeData`）；`buildMetaFromNodesEdges` 增加 zoom/pan 参数支持布局持久化 |
