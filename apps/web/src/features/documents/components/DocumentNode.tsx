...
import { topoDocumentKey, topoDocumentTypeIcon } from '../types/documentTypes'
...
export const DocumentNode: React.FC<DocumentNodeProps> = ({
  ...
  const isActive = topoDocumentKey(node.id) === activeDocumentKey
    ...
    <React.Fragment>
      {isEditing ? (
        ...
        <button
          ...
          onDragStart={readOnly ? undefined : (e) => onDragStart(e, node.id)}
          onDragOver={readOnly ? undefined : (e) => onDragOver(e, node.id)}
          onDragLeave={readOnly ? undefined : onDragLeave}
          onDrop={readOnly ? undefined : (e) => onDrop(e, node.id)}
          ...
          onClick={() => onSelectDocument(topoDocumentKey(node.id))}
          onDoubleClick={(e) => {
            if (hasChildren) {
              onToggleExpand(e, node.id)
            }
          }}
          onContextMenu={readOnly ? undefined : (e) => onContextMenu(e, node.id)}
          title={node.title}
        >
          <span 
