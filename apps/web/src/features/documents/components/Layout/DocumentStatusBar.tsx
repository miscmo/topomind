...
import type { DocumentSaveStatus, DocumentSyncStatus } from '../../types/workspaceTypes'
...
export const DocumentStatusBar = memo(function DocumentStatusBar({
  ...
  const syncView = syncStatus
    ? syncStatus.tone === 'error'
      ? { dot: 'bg-[var(--color-danger)]', textClass: 'text-[var(--color-danger)]' }
      : syncStatus.tone === 'warning'
        ? { dot: 'bg-[var(--color-warning)]', textClass: 'text-[var(--color-warning)]' }
        : syncStatus.tone === 'success'
          ? { dot: 'bg-[var(--color-success)] opacity-80', textClass: 'text-[var(--color-text-muted)]' }
          : { dot: 'bg-[var(--color-primary)]', textClass: 'text-[var(--color-primary)]' }
    : null
      ...
      <div className="flex items-center gap-3">
        ...
        {syncView ? (
          <>
            ...
            <div className="flex items-center gap-1.5 transition-all duration-200">
              <div className={`w-1.5 h-1.5 rounded-full ${syncView.dot}`} />
              <span className={`max-w-[320px] truncate ${syncView.textClass}`} title={syncStatus?.text}>
                {syncStatus?.text}
              </span>
            </div>
          </>
