...
function formatSampleContext(sample: PerformanceSample): string {
  const roomRef = typeof sample.params.roomRef === 'string' ? sample.params.roomRef : ''
  const roomId = typeof sample.params.roomId === 'string' ? sample.params.roomId : ''
  const cardRef = typeof sample.params.cardRef === 'string' ? sample.params.cardRef : ''
  const documentKey = typeof sample.params.documentKey === 'string' ? sample.params.documentKey : ''
  return roomRef || roomId || cardRef || documentKey || sample.module || '未命名上下文'
}
