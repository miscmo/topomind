...
import { useStorage, type TrashItem } from '../../../core/storage'
...
type TrashKBItem = TrashItem | CloudTrashKBItem

function isCloudTrashKBItem(item: TrashKBItem): item is CloudTrashKBItem {
  return typeof (item as CloudTrashKBItem).knowledgeBaseId === 'string'
}
...
export function TrashDialog({ visible, onClose, refreshKBList }: TrashDialogProps) {
  ...
  const [items, setItems] = useState<TrashKBItem[]>([])
  ...
  const loadItems = useCallback(async () => {
    ...
    try {
      if (currentWorkspaceId) {
        ...
        const deletedKbs = snapshot.knowledgeBases
          .filter((kb) => kb.deletedAt)
          .sort((a, b) => String(b.deletedAt).localeCompare(String(a.deletedAt)))
          .map((kb) => ({
            trashName: kb.id,
            knowledgeBaseId: kb.id,
            originalName: kb.name,
            originalPath: kb.name,
            deletedAt: new Date(kb.deletedAt as string).getTime(),
          }))
        setItems(deletedKbs)
        return
      }
  ...
  }, [currentWorkspaceId, storage, visible])

  useEffect(() => {
    void loadItems()
  }, [loadItems])

  const handleRestore = useCallback(async (item: TrashKBItem) => {
    setLoading(true)
    setError('')
    try {
      const restoredName = isCloudTrashKBItem(item)
        ? await (async () => {
            if (!currentWorkspaceId) {
              throw new Error('当前工作区未就绪，暂时无法恢复知识库')
            }
            const restored = await cloudApi.restoreWorkspaceKnowledgeBase(
              currentWorkspaceId,
        ...
        : await storage.restoreTrashKB(item.trashName)
      await refreshKBList()
      await loadItems()
  ...
  }, [currentWorkspaceId, loadItems, refreshKBList, storage])
  ...
  const handleClear = useCallback(async () => {
    ...
    try {
      if (isCloudMode) {
        if (!currentWorkspaceId) {
          throw new Error('当前工作区未就绪，暂时无法清空知识库回收站')
        }
        for (const item of items) {
          if (!isCloudTrashKBItem(item)) {
            throw new Error(`云端知识库缺少必要标识: ${item.originalName}`)
          }
          await cloudApi.purgeWorkspaceKnowledgeBase(currentWorkspaceId, item.knowledgeBaseId)
        }
        await syncWorkspacePullIntoLocalMirror(currentWorkspaceId)
      } else {
  ...
  }, [currentWorkspaceId, isCloudMode, items, storage])
      ...
      <div className={`w-[560px] max-w-[92%] overflow-hidden rounded-2xl border border-[var(--color-border)] bg-surface shadow-[var(--shadow-lg)] ${modalPanelEnterClassName}`}>
        ...
        <div className="p-[20px_24px]">
          ...
          <div className="mb-4 text-[13px] text-[var(--color-text-secondary)]">
            {isCloudMode
              ? '这里显示已删除的知识库。当前云端本地镜像已支持从回收站恢复知识库，也支持对已删除知识库执行永久清空。'
              : '这里显示已删除的知识库。清空时也会一并永久清理已删除节点占用的空间。文档和附件可在对应节点内的回收站中恢复或清空。恢复时如果原名称已存在，会自动添加后缀避免覆盖。'}
          </div>
