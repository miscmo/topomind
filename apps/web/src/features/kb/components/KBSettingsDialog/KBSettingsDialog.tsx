      ...
      <div className={`w-[440px] max-w-[90%] overflow-hidden rounded-2xl border border-[var(--color-border)] bg-surface shadow-[var(--shadow-lg)] ${modalPanelEnterClassName}`} onClick={e => e.stopPropagation()}>
        ...
        <div className="p-[20px_24px]">
          ...
          <div className="mb-4">
            ...
            {isCloudMode && (
              <div className="mb-3 rounded-md bg-[var(--color-primary-soft)] px-3 py-2 text-[12px] leading-snug text-[var(--color-primary)]">
                云模式下当前已支持知识库重命名、封面查看/移除/更换，以及删除知识库；相关变更会同步写入本地镜像并进入同步队列。
              </div>
            )}
          ...
          <div className="mt-6 pt-5 border-t border-[var(--color-border-light)] flex justify-between items-center">
            ...
            <div className="flex gap-3">
              ...
              <button className="bg-[var(--color-accent)] text-white border-none p-[8px_24px] rounded-lg text-[14px] font-semibold cursor-pointer transition-all duration-75 hover:bg-[var(--color-accent-hover)] disabled:opacity-60 disabled:cursor-not-allowed" onClick={handleSave} disabled={loading || !name.trim() || !hasUnsavedChanges}>
                保存设置
              </button>
            </div>
          </div>
