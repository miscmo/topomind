...
export function KnowledgeBaseGrid(props: KnowledgeBaseGridProps) {
  const { kbs, onOpenKB, onOpenSettings, onReorder, disableOpen = false, disableSettings = false } = props
    ...
    <div className="grid grid-cols-3 gap-5 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7 2xl:grid-cols-8">
      {kbs.map((kb) => (
        <div
          ...
          className={`group relative flex flex-col rounded-lg border border-border/60 bg-card transition-all duration-200 overflow-hidden [transform:translateZ(0)]
            ${disableOpen ? 'cursor-default' : 'cursor-pointer hover:-translate-y-1 hover:border-border hover:shadow-md'}
        ...
        >
          <div className="relative aspect-[1/1] w-full overflow-hidden rounded-t-lg bg-muted">
            ...
            <div className={`absolute left-2 top-2 rounded-md p-1 text-white/70 transition-opacity drop-shadow-md ${onReorder ? 'cursor-grab opacity-0 active:cursor-grabbing group-hover:opacity-100' : 'opacity-0'}`}>
