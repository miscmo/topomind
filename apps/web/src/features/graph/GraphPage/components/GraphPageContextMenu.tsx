...
import { Eraser, Maximize2, Palette, PenLine, Plus, Trash2 } from 'lucide-react'
...
interface GraphPageContextMenuProps {
  ...
  onNodeStyle: (nodeId: string) => void
  onNodeClearStyle: (nodeId: string) => void
  onEdgeDelete: (edgeId: string) => void
  onEdgeRelation: (edgeId: string) => void
  onEdgeStyle: (edgeId: string) => void
  onEdgeClearStyle: (edgeId: string) => void
  onClose: () => void
    ...
    : isEdge
      ? [
          {
            ...
            action: async () => {
              if (targetId) await onEdgeRelation(targetId)
              onClose()
            },
          },
          {
          ...
          },
          {
            ...
            action: async () => {
              if (targetId) await onEdgeClearStyle(targetId)
              onClose()
            },
          },
          { separator: true },
      ...
      : [
          ...
          { separator: true },
          {
            ...
            shortcut: 'F2',
            action: async () => {
              if (targetId) await onRenameNode(targetId)
              onClose()
            },
          },
          {
            ...
            icon: <Palette className="w-4 h-4" />,
            action: async () => {
              if (targetId) await onNodeStyle(targetId)
              onClose()
            },
          },
          {
            ...
            icon: <Eraser className="w-4 h-4" />,
            action: async () => {
              if (targetId) await onNodeClearStyle(targetId)
              onClose()
            },
          },
          { separator: true },
