...
function KnowledgeCard({ id, data, selected, dragging, width, height, resizing }: KnowledgeCardProps) {
  // #region debug-point C:knowledge-card-render
  ;(() => {
    const sharedDebugState = (window as any).__nodeDragLagDebug
    if (!sharedDebugState?.active) return
    sharedDebugState.renderCount = (sharedDebugState.renderCount ?? 0) + 1
    sharedDebugState.renderNodeIds = sharedDebugState.renderNodeIds ?? {}
    sharedDebugState.renderNodeIds[id] = (sharedDebugState.renderNodeIds[id] ?? 0) + 1
    if (dragging) {
      sharedDebugState.draggingNodeRenderCount = (sharedDebugState.draggingNodeRenderCount ?? 0) + 1
    }
  })()
  // #endregion
          ...
          >
            {titleEditing ? (
              ...
              <div
                ...
                onDoubleClick={titleEditEnabled ? startTitleEdit : undefined}
                title={titleEditEnabled ? '双击编辑标题' : undefined}
              >
                {data.label}
              </div>
            )}
