import type { DailyRecord, LearningSession } from './learningTrackerStore'
import type { LearningPageType } from './learningTrackerContextStore'
...
export const computeContextAnalyticsFromSessions = (sessions: LearningSession[]): ContextAnalytics => {
  ...
  sessions.forEach(session => {
    ...
    if (context?.kbId) {
      ...
      const currentKb = kbBuckets.get(context.kbId) || { duration: 0, sessionCount: 0 }
      kbBuckets.set(context.kbId, {
  ...
  return {
    ...
    kbDistribution: finalizeContextDistribution('kbId', kbBuckets, kbTrackedDuration),
...
const matchesContextFilter = (session: LearningSession, filter: ContextFilter) => {
  ...
  if (filter.kbId !== 'all' && session.context?.kbId !== filter.kbId) {
    return false
  }
  return true
}
