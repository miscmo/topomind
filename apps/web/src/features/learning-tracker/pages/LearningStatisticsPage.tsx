...
import { readLearningStatsData, readLearningStatsSummary } from '../../../core/learning-stats-backend'
...
export default memo(function LearningStatisticsPage() {
  ...
  const contextFilterLabel = useMemo(() => {
    ...
    if (contextFilter.kbId !== 'all') {
      parts.push(contextFilter.kbId)
    }
    ...
    const load = async () => {
      try {
        const [summary, records] = await Promise.all([
          readLearningStatsSummary(workspaceRoot, SUMMARY_WINDOW_DAYS),
          Promise.all(
            loadedRecordDates.map(async date => {
              const record = await readLearningStatsData(workspaceRoot, date) as DailyRecord | null
  ...
  }, [workspaceRoot, loadedRecordDates, refreshKey])
  ...
  const pageTypeScopedRecords = useMemo(
    () => (
      contextFilter.pageType === 'all'
        ? liveRangeRecords
        : filterDailyRecordsByContext(liveRangeRecords, { pageType: contextFilter.pageType, kbId: 'all' })
    ),
  ...
  useEffect(() => {
    if (contextFilter.kbId === 'all') return
    if (kbOptions.includes(contextFilter.kbId)) return
    setContextFilter(filter => ({ ...filter, kbId: 'all' }))
  }, [contextFilter.kbId, kbOptions])
        ...
        <div className="mb-6 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-5">
          ...
          <div className="mt-4 flex flex-col gap-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <select
                value={contextFilter.kbId}
                onChange={(event) => setContextFilter(filter => ({ ...filter, kbId: event.target.value }))}
