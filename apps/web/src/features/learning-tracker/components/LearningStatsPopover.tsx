...
export const LearningStatsPopover = memo(function LearningStatsPopover({
  ...
  const fetchHeatmap = async () => {
    ...
    setLoading(true)
    try {
      const dataMap = await readLearningStatsSummary(workspaceRoot, PAST_DAYS)
  ...
  useEffect(() => {
    ...
    const loadSelectedDay = async () => {
      try {
        let dayData = await readLearningStatsData(workspaceRoot, selectedDate) as DailyRecord | null
  ...
  }, [workspaceRoot, selectedDate, refreshKey, todayDuration, currentSession, todayStr])
