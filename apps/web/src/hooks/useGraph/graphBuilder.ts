...
export async function buildNodes(
  ...
  const roomGraph = graphMetaToRoomGraph(roomRef, meta)
  const normalizedChildren = Object.values(roomGraph.nodes).map((roomNode) => {
    const nodeId = roomNode.id || roomNode.cardRef
    const childRef = resolveRoomChildRef(roomRef, roomNode.cardRef || nodeId)
    return [nodeId, childRef, roomNode] as [string, string, RoomGraphNode]
  })
  ...
  const nodeInfoResults = await mapWithConcurrency(
    ...
    async ([, childRef]) => {
      const childCount = await storage.countChildren(childRef).catch(() => 0)
      const hasDetail = storage.listTopoDocuments 
        ? await storage.listTopoDocuments(childRef).then(docs => docs.length > 0).catch(() => false)
        : false
      return { childCount, hasContent: false, hasDetail }
    }
  )

  return normalizedChildren.map(([nodeId, childRef, roomNode], i) => {
    ...
    return {
      ...
      data: {
        label: roomNode.name || basenameRef(childRef),
        parent: roomRef || kbId || undefined,
...
/** Build React Flow edges from graph metadata */
export function buildEdges(meta: GraphMeta, roomRef = ''): KnowledgeEdge[] {
  const roomGraph = graphMetaToRoomGraph(roomRef, meta)
