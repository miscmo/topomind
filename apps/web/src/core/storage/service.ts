...
import { CloudApiError } from '../http-client'
...
import type { AttachmentUploadSyncContext } from './local-types'
...
function mergeVaultConfig(base: VaultConfig, patch: VaultConfig): VaultConfig {
  const nextConfig = { ...base, ...patch }
  if (patch.defaultEdgeStyle) {
    nextConfig.defaultEdgeStyle = { ...base.defaultEdgeStyle, ...patch.defaultEdgeStyle }
  }
  if (patch.defaultNodeStyle) {
    nextConfig.defaultNodeStyle = { ...base.defaultNodeStyle, ...patch.defaultNodeStyle }
  }
  if (patch.defaultNodeSize) {
    nextConfig.defaultNodeSize = { ...base.defaultNodeSize, ...patch.defaultNodeSize }
  }
  if (patch.defaultEditorStyle) {
    nextConfig.defaultEditorStyle = { ...base.defaultEditorStyle, ...patch.defaultEditorStyle }
  }
  if (patch.nodeSizeLimits) {
    nextConfig.nodeSizeLimits = { ...base.nodeSizeLimits, ...patch.nodeSizeLimits }
  }
  if (patch.nodeBadgeSize !== undefined) {
    nextConfig.nodeBadgeSize = patch.nodeBadgeSize
  }
  if (patch.kbCovers !== undefined) {
    nextConfig.kbCovers = patch.kbCovers
  }
  if (patch.kbCoverOffsets !== undefined) {
    nextConfig.kbCoverOffsets = patch.kbCoverOffsets
  }
...
export function createStore(backend: StorageBackend) {
  const layoutSaveCoordinator = new SaveCoordinator<GraphMeta>({
    ...
    save: async (roomRef, meta) => {
      await backend.writeLayout(roomRef, normalizeGraphMeta(meta))
    },
    onError: (error, roomRef) => {
      logger.catch('Store.saveCoordinator', `保存布局失败: ${roomRef}`, error)
    },
  })
  ...
  const store = {
    ...
    async isValidVault(dirPath: string) {
      try {
        ...
        return { valid, selectedPath: valid ? dirPath : null, error: valid ? undefined : error || '不是有效的工作目录' }
    ...
    async deleteKB(kbId: string) {
      try {
        await backend.deleteKB(kbId)
      } catch (e) { logger.catch('Store.deleteKB', `删除知识库失败: ${kbId}`, e); throw e }
    },
    async renameKB(kbId: string, newName: unknown) {
      const safeName = ensureValidName(newName, '知识库名称')
      try {
        await backend.renameKB(kbId, safeName)
      } catch (e) { logger.catch('Store.renameKB', `重命名知识库失败: ${kbId} -> ${newName}`, e); throw e }
    },
    async listCards(parentRef: string) {
      try {
        return await backend.listCards(parentRef)
      } catch (e) { logger.catch('Store.listCards', `列出卡片失败: ${parentRef}`, e); throw e }
    },
    async createCard(parentRef: string, cardName: unknown) {
      const safeName = ensureValidName(cardName, '卡片目录名')
      try {
        const cardInfo = await backend.createCard(parentRef, safeName)
        return cardInfo.ref
      } catch (e) { logger.catch('Store.createCard', `创建卡片失败: ${parentRef}/${cardName}`, e); throw e }
    },
    ...
    async renameCard(cardPath: string, newName: unknown) {
      ...
      try {
        const parentRef = cardPath.includes('/') ? cardPath.slice(0, cardPath.lastIndexOf('/')) : ''
        const siblings = await backend.listCards(parentRef)
    ...
    async importAttachment(
      ...
      try { return await backend.importAttachment(cardPath, sourceFilePath, targetFileName, syncContext, uploadTicketJson) } catch (e) { logger.catch('Store.importAttachment', `导入附件失败: ${cardPath}`, e); throw e }
    },
    ...
    async writeAttachmentBase64(
      ...
      try { return await backend.writeAttachmentBase64(cardPath, fileName, mimeType, base64, syncContext, uploadTicketJson) } catch (e) { logger.catch('Store.writeAttachmentBase64', `写入附件失败: ${cardPath}/${fileName}`, e); throw e }
    },
    async downloadAttachment(
      ...
      try { return await backend.downloadAttachment(cardPath, url, targetFileName, syncContext, uploadTicketJson) } catch (e) { logger.catch('Store.downloadAttachment', `下载附件失败: ${cardPath}`, e); throw e }
    },
    ...
    writeLayout: async (roomRef: string, meta: GraphMeta) => {
      try { await backend.writeLayout(roomRef, normalizeGraphMeta(meta)) } catch (e) { logger.catch('Store.writeLayout', `写入布局失败: ${roomRef}`, e); throw e }
    },
    readLayout: async (roomRef: string): Promise<GraphMeta> => {
      try { return normalizeGraphMeta(await backend.readLayout(roomRef)) } catch (e) { logger.catch('Store.readLayout', `读取布局失败: ${roomRef}`, e); throw e }
    },
    async saveLayout(roomRef: string, meta: GraphMeta) {
      try { await backend.writeLayout(roomRef, normalizeGraphMeta(meta)) } catch (e) { logger.catch('Store.saveLayout', `保存布局失败: ${roomRef}`, e); throw e }
    },
    saveGraphDebounced(roomRef: string, buildMetaFn: () => GraphMeta, onSaved?: () => void): Promise<void> {
      if (!roomRef) return Promise.resolve()
      return layoutSaveCoordinator.schedule(roomRef, buildMetaFn, onSaved)
    },
    flushGraphSave(roomRef: string, buildMetaFn: () => GraphMeta, onSaved?: () => void): Promise<void> {
      if (!roomRef) return Promise.resolve()
      return layoutSaveCoordinator.flush(roomRef, buildMetaFn, onSaved)
    },
    ...
    async writeConfig(config: VaultConfig) {
      const runWrite = async () => {
        const persistConfig = async (baseConfig: VaultConfig) => {
          const next = mergeVaultConfig(baseConfig, config)
        ...
        try {
          ...
          if (e instanceof CloudApiError && e.status === 409) {
            const freshConfig = normalizeConfig(await backend.readConfig())
            cachedConfig = freshConfig
            cachedConfigTimestamp = Date.now()
            await persistConfig(freshConfig)
            return
          }
          throw e
        }
      ...
      const queuedWrite = configWriteQueue.then(runWrite, runWrite)
      configWriteQueue = queuedWrite.then(() => undefined, () => undefined)
