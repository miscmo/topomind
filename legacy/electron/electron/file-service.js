...
const TOPO_DOCUMENT_PRIMARY_EXTENSIONS = {
  smart: '.tdoc.json',
  mindmap: '.tmind.json',
...
function _fs_topoDocumentExtensions(type) {
  return TOPO_DOCUMENT_EXTENSIONS[_fs_normalizeTopoDocumentType(type)];
}

function _fs_topoDocumentPrimaryExtension(type) {
  return TOPO_DOCUMENT_PRIMARY_EXTENSIONS[_fs_normalizeTopoDocumentType(type)];
}

function _fs_detectTopoDocumentTypeByFileName(fileName) {
  var matchedType = null;
  Object.keys(TOPO_DOCUMENT_EXTENSIONS).forEach(function(candidateType) {
    if (matchedType) return;
    if (_fs_topoDocumentExtensions(candidateType).some(function(extension) { return fileName.endsWith(extension); })) {
...
function _fs_normalizeTopoDocumentFileName(type, fileNameInput) {
  var normalizedType = _fs_normalizeTopoDocumentType(type);
  var raw = String(fileNameInput || '').trim().replace(/\\/g, '/').replace(/^\.\/+/, '').replace(/^\/+|\/+$/g, '');
  var fileName = nodePath.basename(raw);
  var extensions = _fs_topoDocumentExtensions(normalizedType);
  if (!extensions.some(function(extension) { return fileName.endsWith(extension); })) {
...
function _fs_topoDocumentAbsolutePath(rootDir, cardPath, item) {
  ...
  var normalizedFileName = _fs_normalizeTopoDocumentFileName(item.type, item.fileName);
  var resolvedPath = nodePath.resolve(docsDir, normalizedFileName);
  var rel = nodePath.relative(docsDir, resolvedPath);
  if (rel === '..' || rel.startsWith('..' + nodePath.sep) || nodePath.isAbsolute(rel)) {
    throw new Error('文档文件名越界: ' + item.fileName);
  }
  return resolvedPath;
}

function _fs_topoDocumentFileNameKey(item) {
  return item.type + ':' + item.fileName;
}

function _fs_topoDocumentTitleFromFile(type, filePath, fileName) {
  var fallback = fileName;
  _fs_topoDocumentExtensions(type).forEach(function(extension) {
    if (fallback === fileName && fileName.endsWith(extension)) {
      fallback = fileName.slice(0, -extension.length);
    }
  });
  try {
...
function _fs_normalizeTopoDocumentManifestItem(id, item) {
  if (!_fs_isPlainObject(item)) return null;
  var type = _fs_normalizeTopoDocumentType(item.type);
  var fileName = _fs_normalizeTopoDocumentFileName(type, item.fileName);
  var normalized = {
    id: id,
    type: type,
  ...
  if (item.originalParentId) normalized.originalParentId = String(item.originalParentId);
  if (item.originalDocumentId) normalized.originalDocumentId = String(item.originalDocumentId);
  return normalized;
}

function _fs_normalizeTopoDocumentManifest(raw) {
  var documents = {};
  if (_fs_isPlainObject(raw) && raw.version === 2 && _fs_isPlainObject(raw.documents)) {
    Object.keys(raw.documents).forEach(function(id) {
      try {
        var normalizedItem = _fs_normalizeTopoDocumentManifestItem(id, raw.documents[id]);
        if (normalizedItem) {
          documents[id] = normalizedItem;
        }
      } catch (e) {}
    });
  }
...
function _fs_scanTopoDocumentFiles(rootDir, cardPath, previousManifest) {
  ...
  nodeFs.readdirSync(docsDir, { withFileTypes: true })
    ...
    .forEach(function(entry) {
      ...
      if (fileName === TOPO_DOCUMENT_MANIFEST) return;

      var type = _fs_detectTopoDocumentTypeByFileName(fileName);
      if (!type) return;

      var filePath = nodePath.join(docsDir, fileName);
      var stat = nodeFs.statSync(filePath);
      var previous = previousByFileName.get(type + ':' + fileName);
...
function _fs_reconcileTopoDocumentManifest(rootDir, cardPath, manifest) {
  ...
  var scannedByFileName = new Map();
  Object.keys(scanned).forEach(function(id) {
    var item = scanned[id];
    scannedByFileName.set(_fs_topoDocumentFileNameKey(item), item);
  });
  var usedFileNames = new Set();
  var documents = {};
  var removed = 0;
  Object.keys(normalized.documents).forEach(function(id) {
    var item = normalized.documents[id];
    var key = _fs_topoDocumentFileNameKey(item);
    var scannedItem = scannedByFileName.get(key);
    if (!scannedItem) {
      removed += 1;
      return;
    }
  ...
  Object.keys(scanned).forEach(function(id) {
    var item = scanned[id];
    var key = _fs_topoDocumentFileNameKey(item);
    if (usedFileNames.has(key)) return;
    added += 1;
    documents[id] = item;
  });
...
function _fs_exportTopoDocument(rootDir, cardPath, documentId) {
  ...
  var fileName = nodePath.basename(found.item.fileName);
  return {
    fileName: fileName,
...
function _fs_createTopoDocument(rootDir, cardPath, input) {
  ...
  var fileName = id + _fs_topoDocumentPrimaryExtension(type);
  var filePath = nodePath.join(docsDir, fileName);
...
function _fs_restoreTrashTopoDocument(rootDir, cardPath, trashName) {
  ...
  var safeFileName = _fs_normalizeTopoDocumentFileName(type, topoItem.fileName || (nextId + TOPO_DOCUMENT_EXTENSIONS[type]));
  if (manifest.documents[nextId] || Object.values(manifest.documents).some(function(existing) { return existing.fileName === safeFileName; })) {
    safeFileName = nextId + TOPO_DOCUMENT_EXTENSIONS[type];
  }
  _fs_ensureDir(docsDir);
  ...
  var target = nodePath.join(docsDir, safeFileName);
  if (nodeFs.existsSync(target)) target = _fs_uniqueFilePath(docsDir, safeFileName);
  nodeFs.copyFileSync(source, target);
  var restoredFileName = nodePath.basename(target);
